...
export async function onRequest(context) {
  ...
  try {
    ...
    if (subPath === "/submit" && request.method === "POST") {
      ...
      const { grade, className, type, totalStudents, questions } = body;
      if (!grade || !className || !type || !questions) {
        return json({ error: "Missing fields: grade, className, type, questions" }, 400);
      }
    ...
    if (subPath === "/export") {
      ...
      // CSV header
      let header = "年級,班別,類型,總人數,提交時間";
      for (let i = 1; i <= 5; i++) {
        header += `,Q${i}-5分,Q${i}-4分,Q${i}-3分,Q${i}-2分,Q${i}-1分,Q${i}-平均分`;
      }
      const rows = [header];
      for (const r of results) {
        const row = [r.grade, r.className, r.type === "student" ? "學生" : "教師", r.totalStudents || "", r.timestamp];
        for (let i = 1; i <= 5; i++) {
